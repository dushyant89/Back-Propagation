...
public class Classifier_NAND {
	...
	public static void main(String[] args) throws NumberFormatException, IOException 
	{
		...
		for(int i=0;i<rows;i++)
		{
		 ...
		 //generating the nand table specific for every function
        for(int i=0;i<rows;i++)
        {
            nand[i][cols-1]=1;
            for(int j=0;j<cols-1;j++)
            {
               nand[i][cols-1]&=nand[i][j];
            }
           nand[i][cols-1]=1^nand[i][cols-1];
        }
		...
		outer:while(true) //should not run till eternity :D
		{
			...
			for(int r=0;r<rows;r++)
			{
				for(int i=0;i<hidden_layer;i++)
				{
					...
					{    
						for(int n=0;n<no_neurons;n++)
						{
							...
							for(int j=0;j<cols-1;j++)
							{
								net+=nand[r][j]*layers[i].neurons[n].to[to_count++].weight;
							}
				...
				int t=nand[r][cols-1]; //the expected output
						...
						else if(i==0)  //the first layer
						{
							for(int n=0;n<no_neurons;n++) //neurons for the layer whose incoming weights are to be corrected
							{
								...
								for(int j=0;j<layers[i].neurons[n].to.length;j++) //links coming towards the neuron whose weights are to be corrected
								{
									...
									{	
									  layers[i].neurons[n].error*=(1-o)*o;
									  layers[i].neurons[n].to[j].weight+=learning_rate*layers[i].neurons[n].error*nand[r][j] + momentum*layers[i].neurons[n].to[j].delta_weight;  
									  layers[i].neurons[n].to[j].delta_weight=learning_rate*layers[i].neurons[n].error*nand[r][j];
									}
								}
