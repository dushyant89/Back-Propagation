...
public class Classifier_Palindrome {
	...
	public static void main(String[] args) throws NumberFormatException, IOException 
	{
		...
		int [][]palindrome=new int[rows][cols];
		int number;
		...
		for(int i=0;i<rows;i++)
		{
			...
			while(number!=0)
			{
				palindrome[i][--input]=number%2;
				number=number/2;
			}
			while(input >0)
			{
				palindrome[i][--input]=0;
			}
		}
		//generating the palindrome table generic for any input
        for(int i=0;i<rows;i++)
        {
            palindrome[i][cols-1]=0;
            int j;
            for(j=0;j<(cols-1)/2;j++)
            {
               if(palindrome[i][j]==palindrome[i][cols-2-j])
               {
                   continue;
               }
               else
				...
				for(int i=0;i<hidden_layer;i++)
				{
					...
					{    
						for(int n=0;n<no_neurons;n++)
						{
							...
							{
								net+=palindrome[r][j]*layers[i].neurons[n].to[to_count++].weight;
							}
